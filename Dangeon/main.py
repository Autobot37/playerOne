...
class BSP:
  ...
  def draw(self):
    pass
    #self.engine.map_renderer.draw_node(self.root_node_id)
  ...
  def update2(self):
    ...
    if is_on_back:
      self.root_node_id = node.back_child_id
    else:
      self.root_node_id = node.front_child_id
...
class Engine:
  ...
  def on_init(self):
    self.wad_data = WADData(map_name='E1M1')
