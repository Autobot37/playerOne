...
class WADReader:
    ...
    def read_linedef(self,offset):
        linedef = Lindedef()
        linedef.start_vertex_id = self.read_2_bytes(offset, byte_format='H')
        linedef.end_vertex_id = self.read_2_bytes(offset+2, byte_format='H')
        linedef.flags = self.read_2_bytes(offset+4, byte_format='H')
        linedef.line_type = self.read_2_bytes(offset+6, byte_format='H')
        linedef.sector_tag = self.read_2_bytes(offset+8, byte_format='H')
        linedef.front_sidedef_id = self.read_2_bytes(offset+10, byte_format='H')
        linedef.back_sidedef_id = self.read_2_bytes(offset+12, byte_format='H')
        return linedef
